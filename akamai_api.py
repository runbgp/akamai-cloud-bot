...
class AkamaiCloudAPI:
    ...
    def create_instance(self, 
                        ...
                        image: str, 
                        root_pass: str, 
                        type: str = "g6-nanode-1") -> Dict[str, Any]:
        """
    ...
    def get_instances(self) -> List[Dict[str, Any]]:
        ...
        response = requests.get(f"{self.BASE_URL}/linode/instances", headers=self.headers)
        response.raise_for_status()
        return response.json().get("data", [])
    
    def get_instance(self, instance_id: int) -> Dict[str, Any]:
        """Get details for a specific Akamai Cloud instance."""
        response = requests.get(f"{self.BASE_URL}/linode/instances/{instance_id}", headers=self.headers)
        response.raise_for_status()
        return response.json()
    
    def delete_instance(self, instance_id: int) -> bool:
        """Delete an Akamai Cloud instance."""
        response = requests.delete(f"{self.BASE_URL}/linode/instances/{instance_id}", headers=self.headers)
